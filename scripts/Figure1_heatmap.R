...
#all_quantile <- all_quantile[order(all_quantile$Study_group),]

ACPA_neg_df <- filter(all_quantile, all_quantile$Study_group == 'ACPA_negative_RA')
#View(ACPA_neg_df)


ACPA_pos_df <- filter(all_quantile, all_quantile$Study_group == 'ACPA_positive_RA')
#View(ACPA_pos_df)


control_df <- filter(all_quantile, all_quantile$Study_group == 'control')
#View(control_df)
...
rownames(all_quantile_df) <- all_quantile$Sample_ID
...
colnames(colors_df) <- c("Study_group")
colors_df[1:92,1] <- all_quantile$Study_group
rownames(colors_df) <- all_quantile$Sample_ID
#View(colors_df)
