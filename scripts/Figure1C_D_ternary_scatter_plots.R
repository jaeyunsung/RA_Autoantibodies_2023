...
for (i in 1:num_features1) {
  ...
  if(absolute_ACPA_pos > 1 | absolute_ACPA_neg > 1){
    all_RA_log_changes[i,7] <- "Different in RA"
  }
}
...
ternary_plot <- ggtern(data = all_mean_values_df, aes(x=ACPA_neg, y = ACPA_pos, z=Control))+geom_point()+theme_bw()+theme_rgbw()

pdf("../output/ternary_plot.pdf")
ternary_plot
dev.off() 
